...
bool SoftwareGraphics::Create() {
    ...
    if (m_FbWidth <= 0 || m_FbHeight <= 0) {
        SW_LOGE("Invalid dimensions: %dx%d", m_FbWidth, m_FbHeight);
        return false;
    }
    ANativeWindow_setBuffersGeometry(m_Window, m_FbWidth, m_FbHeight, WINDOW_FORMAT_RGBA_8888);
    try {
        m_Framebuffer.resize((size_t)m_FbWidth * (size_t)m_FbHeight, 0);
    } catch (...) {
        SW_LOGE("Failed to allocate framebuffer");
        return false;
    }
...
void SoftwareGraphics::PrepareFrame(bool resize) {
    if (resize) {
        ...
        if (m_FbWidth > 0 && m_FbHeight > 0) {
            ANativeWindow_setBuffersGeometry(m_Window, m_FbWidth, m_FbHeight, WINDOW_FORMAT_RGBA_8888);
            m_Framebuffer.resize((size_t)m_FbWidth * (size_t)m_FbHeight);
        }
    }
...
void SoftwareGraphics::Render(ImDrawData *drawData) {
    if (!drawData || drawData->CmdListsCount == 0 || m_Framebuffer.empty())
        return;
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        if (!vtxBuffer || !idxBuffer) continue;
        ...
        for (int cmdIdx = 0; cmdIdx < cmdList->CmdBuffer.Size; cmdIdx++) {
            ...
            if (clipRect.x >= m_FbWidth || clipRect.y >= m_FbHeight || clipRect.z <= 0 || clipRect.w <= 0)
                continue;
...
void SoftwareGraphics::PrepareShutdown() {
    ...
    for (ImTextureData *tex : platform_io.Textures) {
        if (tex) SoftwareDestroyTexture(tex);
    }
...
BaseTexData *SoftwareGraphics::LoadTexture(BaseTexData *tex, void *pixel_data) {
    if (!tex || !pixel_data || tex->Width <= 0 || tex->Height <= 0) return nullptr;
    ...
    texData->Pixels.resize(pixelCount);
    memcpy(texData->Pixels.data(), pixel_data, pixelCount * 4);
...
void SoftwareGraphics::SoftwareUpdateTexture(ImTextureData *tex) {
    ...
    if (tex->Status == ImTextureStatus_WantCreate) {
        if (tex->TexID != ImTextureID_Invalid || tex->BackendUserData != nullptr) {
        ...
        size_t pixelCount = (size_t)tex->Width * (size_t)tex->Height;
        swTex->Pixels.resize(pixelCount);

        if (tex->Format == ImTextureFormat_RGBA32) {
            memcpy(swTex->Pixels.data(), tex->GetPixels(), pixelCount * 4);
        ...
        if (tex->Format == ImTextureFormat_RGBA32) {
            for (ImTextureRect &r : tex->Updates) {
                ...
                for (int y = 0; y < r.h; y++) {
                    ...
                    uint32_t *dstRow = &swTex->Pixels[(size_t)(r.y + y) * swTex->TexWidth + r.x];
    ...
    const ImVec4 &clipRect)
{
    ...
    int minX = imaxVal((int)std::floor(minXf), imaxVal((int)clipRect.x, 0));
    int minY = imaxVal((int)std::floor(minYf), imaxVal((int)clipRect.y, 0));
    int maxX = iminVal((int)std::ceil(maxXf), iminVal((int)clipRect.z, m_FbWidth));
    int maxY = iminVal((int)std::ceil(maxYf), iminVal((int)clipRect.w, m_FbHeight));
    ...
    const float A01 = (p1.y - p2.y) * invDenom;
    const float B01 = (p2.x - p1.x) * invDenom;
    const float A12 = (p2.y - p0.y) * invDenom;
    const float B12 = (p0.x - p2.x) * invDenom;
    ...
    const bool hasTex = tex && !tex->Pixels.empty() && tex->TexWidth > 0 && tex->TexHeight > 0;
    ...
    const float texWf = (float)(texW > 1 ? texW - 1 : 1);
    const float texHf = (float)(texH > 1 ? texH - 1 : 1);
        ...
        for (int x = minX; x < maxX; x++) {
            ...
            if (w0 >= 0.0f && w1 >= 0.0f && w2 >= 0.0f) {
                ...
                if (texPixels) {
                    ...
                    int tx = (int)(u * texWf);
                    int ty = (int)(v * texHf);
                    tx = iminVal(tx, texW - 1);
                    ty = iminVal(ty, texH - 1);
...
uint32_t SoftwareGraphics::SampleTexture(const SoftwareTextureData *tex, float u, float v) {
    if (!tex || tex->Pixels.empty() || tex->TexWidth <= 0 || tex->TexHeight <= 0) return 0;
    u = fclamp(u, 0.0f, 1.0f);
    v = fclamp(v, 0.0f, 1.0f);
    int tx = (int)(u * (tex->TexWidth - 1));
    int ty = (int)(v * (tex->TexHeight - 1));
    tx = imaxVal(0, iminVal(tx, tex->TexWidth - 1));
    ty = imaxVal(0, iminVal(ty, tex->TexHeight - 1));
