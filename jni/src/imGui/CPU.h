...
namespace CPU {
    ...
    inline void Tick() {
        ...
        if (last_tick.time_since_epoch().count() != 0) {
            ...
            float sum = 0.f;
            int count = 0;
            for (float s : samples) if (s > 0.f) { sum += s; count++; }
            float avg_ms = (count > 0) ? (sum / (float)count) : 16.66f;
    ...
    inline void Graph() {
        ImGui::PlotLines("##frametimes", samples, HISTORY, head, nullptr, 0.f, 33.33f, ImVec2(-1, 50));
    }
}
